...
c.find("#cropBody").removeClass("d-none");c.find("#cropFooter").removeClass("d-none");c.find("#file_form_group").addClass("d-none");c.find("#id_content").parent().removeClass("d-none")});d.attr("src",a.target.result)};a.readAsDataURL(this.files[0])}})};
EMarkdownEditor.insertCode=(a)=>{a.preventDefault();if(a=EMarkdownEditor.get(a.data.widgetId)){var b=a.codeInput.val(),c=a.selectCode.val();if(0<b.length)return b="\n```"+c+"\n"+b+"\n```\n",EMarkdownEditor.restoreSelection(),a.textarea.insertAtCaret(b),a.codeInput.val(""),!0}return!1};
