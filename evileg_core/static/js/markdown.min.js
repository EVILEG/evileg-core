...
"lang-sh":"text/x-sh","lang-xhtml":"htmlmixed","lang-xml":"text/html","lang-xsl":"text/html"};
jQuery.fn.extend({insertAtCaret(a){return this.each(function(c){if(document.selection)this.focus(),document.selection.createRange().text=a,this.focus();else if(this.selectionStart||"0"==this.selectionStart){c=this.selectionStart;var b=this.selectionEnd,d=this.scrollTop;this.value=this.value.substring(0,c)+a+this.value.substring(b,this.value.length);this.focus();this.selectionStart=c+a.length;this.selectionEnd=c+a.length;this.scrollTop=d}else this.value+=a,this.focus()})}});
var EMarkdownEditor=function(a,c,b){c=void 0===c?"":c;b=void 0===b?"":b;this.id=a;this.textarea=jQuery("#"+a);this.tabPreviewLink=jQuery("#"+a+"_tab_preview_link");this.tabPreviewLink.bind("shown.bs.tab",{widgetId:a},EMarkdownEditor.updatePreview);this.linkDialog=jQuery("#"+a+"_add_link_dialog");this.addLinkBtn=jQuery("#"+a+"_add_link_btn");this.addLinkBtn.bind("click",{dialog:this.linkDialog},EMarkdownEditor.showDialog);this.insertLinkBtn=jQuery("#"+a+"_insert_link_btn");this.insertLinkBtn.bind("click",
{widgetId:a},EMarkdownEditor.insertLink);this.linkText=jQuery("#"+a+"_link_text");this.linkUrl=jQuery("#"+a+"_link_url");this.codeDialog=jQuery("#"+a+"_code_dialog");this.addCodeBtn=jQuery("#"+a+"_add_code_btn");this.addCodeBtn.bind("click",{dialog:this.codeDialog},EMarkdownEditor.showDialog);this.insertCodeBtn=jQuery("#"+a+"_insert_code_btn");this.insertCodeBtn.bind("click",{widgetId:a},EMarkdownEditor.insertCode);this.selectCode=jQuery("#"+a+"_select_code");this.selectCode.bind("change",{widgetId:a},
EMarkdownEditor.onSelectCode);this.codeInput=jQuery("#"+a+"_code_input");this.mirrorEditor=eval(a+"_code_input_codemirror");this.uploadLink=c;this.addImageBtn=jQuery("#"+a+"_add_image_btn");this.addImageBtn.bind("click",{widgetId:a},EMarkdownEditor.showUploadDialog);this.uploadFileLink=b;this.addFileBtn=jQuery("#"+a+"_add_file_btn");this.addFileBtn.bind("click",{widgetId:a},EMarkdownEditor.showUploadFileDialog);this.addCutBtn=jQuery("#"+a+"_add_cut_btn");this.addCutBtn.bind("click",{widgetId:a},EMarkdownEditor.insertCut)};
EMarkdownEditor.onSelectCode=(a)=>{a=EMarkdownEditor.get(a.data.widgetId);a.mirrorEditor.setOption("mode",LANGUAGES[a.selectCode.val()])};EMarkdownEditor.insertCut=(a)=>{a.preventDefault();(a=EMarkdownEditor.get(a.data.widgetId))&&a.textarea.insertAtCaret("\n___\n");return!1};
...
e&&d.getCroppedCanvas().toBlob((a)=>{e.set("file",a,"photo.jpg");jQuery.ajax({url:c.uploadLink,type:"POST",data:e,cache:!1,processData:!1,contentType:!1,success(a){a.result&&(a="\n[!["+a.description+"]("+a.src+")]("+a.url+")\n",EMarkdownEditor.restoreSelection(),c.textarea.insertAtCaret(a),b.find("#upload-size-warning").addClass("d-none"),b.modal("hide"))},error(a,c,d){413===a.status&&b.find("#upload-size-warning").removeClass("d-none")}})});return!1});b.find("#upload-size-warning").addClass("d-none");
b.find("#cropBody").removeClass("d-none");b.find("#cropFooter").removeClass("d-none");b.find("#file_form_group").addClass("d-none");b.find("#id_content").parent().removeClass("d-none")});d.attr("src",a.target.result)};a.readAsDataURL(this.files[0])}});b.on("hidden.bs.modal",()=>{b.remove()})};
EMarkdownEditor.initUploadFileDialog=(a)=>{var c=EMarkdownEditor.get(a),b=jQuery("#uploadFileDialog");b.find("#id_file").change(function(a){this.files&&this.files[0]&&(b.find("#file-upload-submit").click((a)=>{a.preventDefault();(a=new FormData(b.find("#formUpload").get(0)))&&jQuery.ajax({url:c.uploadFileLink,type:"POST",data:a,cache:!1,processData:!1,contentType:!1,success(a){a.result&&(a="\n[!["+a.name+"](/static/images/file.svg) "+a.name+"]("+a.url+")\n",EMarkdownEditor.restoreSelection(),c.textarea.insertAtCaret(a),
...
EMarkdownEditor.insertCode=(a)=>{a.preventDefault();if(a=EMarkdownEditor.get(a.data.widgetId)){a.codeInput.val(a.mirrorEditor.getValue());var c=a.codeInput.val(),b=a.selectCode.val();if(0<c.length)return c="\n```"+b+"\n"+c+"\n```\n",EMarkdownEditor.restoreSelection(),a.textarea.insertAtCaret(c),a.codeInput.val(""),a.mirrorEditor.getDoc().setValue(""),!0}return!1};
EMarkdownEditor.insertLink=(a)=>{a.preventDefault();if(a=EMarkdownEditor.get(a.data.widgetId)){var c=a.linkUrl.val();if(0<c.length){var b=a.linkText.val(),d=void 0,d=0<b.length?"["+b+"]("+c+")":"["+c+"]("+c+")";EMarkdownEditor.restoreSelection();a.textarea.insertAtCaret(d);a.linkUrl.val("");a.linkText.val("");return!0}}return!1};EMarkdownEditor.showDialog=(a)=>{a.data.dialog.modal("show");return!1};EMarkdownEditor.create=(a,c,b)=>{c=new EMarkdownEditor(a,void 0===c?"":c,b);return allEditors[a]=c};
...
EMarkdownEditor.restoreSelection=(a)=>{if(a)if(window.getSelection){var c=window.getSelection();c.removeAllRanges();c.addRange(a)}else document.selection&&a.select&&a.select()};EMarkdownEditor.updatePreview=(a)=>{var c=a.data.widgetId;$.ajax({url:"/evileg_core/markdown/",type:"POST",data:{content:jQuery("#"+c).val()},dataType:"json",success(a){jQuery("#"+c+"_preview").html(a.preview);prettyPrint()}})};
