...
{widgetId:a},EMarkdownEditor.showUploadFileDialog);a=replaceAll(a,"-","_");try{this.mirrorEditor=eval(a+"_code_input_codemirror"),this.mirrorEditor.on("change",(a)=>{d.codeInput.val(a.getValue())}),this.mirrorEditor.setOption("extraKeys",{"Ctrl-/":"toggleComment"})}catch(f){console.log("Code editor not found. id:",a+"_code_input_codemirror")}try{this.markdownMirrorEditor=eval(a+"_codemirror"),this.markdownMirrorEditor.on("change",(a)=>{d.textarea.val(a.getValue())}),this.markdownMirrorEditor.setOption("extraKeys",
{F11(a){d.fullScreen()},Esc(a){jQuery("body").removeClass("overflow-hidden");d.widget.removeClass("markdown-fullscreen");d.fullscreen=!1;d.fullScreenButton.find("span").removeClass("mdi-fullscreen-exit");d.fullScreenButton.find("span").addClass("mdi-fullscreen")},"Ctrl-/":"toggleComment"})}catch(e){console.log("Markdown editor not found. id:",a+"_codemirror")}};
...
EMarkdownEditor.updatePreview=(a)=>{var c=a.data.widgetId;$.ajax({url:"/evileg_core/markdown/",type:"POST",data:{content:jQuery("#"+c).val()},dataType:"json",success(a){jQuery("#"+c+"_preview").html(a.preview);PR.prettyPrint()}})};
